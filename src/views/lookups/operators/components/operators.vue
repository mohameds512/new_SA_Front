<template>
  ...
  <!--            </b-link>-->
      ...
      <template #action>
        <b-button :to="{ name: 'operators-add' }" v-b-tooltip.hover="$t('Global.add')" variant="primary" class="btn-icon mr-1">
  ...
  console.log('dddd');
    ...
    methods: {
      ...
      open(id) {
        window.open(this.$router.resolve({ name: 'operators-edit', params: { id } }).href, '_self');
      },
      refresh(query) {
      ...
      remove(userId) {
        this.$swal({
          ...
          if (result.value) {
            this.$store.dispatch('operators/remove', userId).then(_ => {
              this.refresh();
              this.$swal({
                icon: 'success',
                text: "Operator is deleted successfully",
                showConfirmButton: false,
                timer: 1500,
              });
            });
