<template>
  ...
  <!--            </b-link>-->
      ...
      <template #action>
        <b-button  :to="{ name: 'AircraftStatusAdd' }" v-b-tooltip.hover="$t('Global.add')" variant="primary" class="btn-icon mr-1">
  ...
  export default {
    ...
    computed: {
      ...mapGetters({
        items: 'aircraft_status/items',
        total: 'aircraft_status/total',
        load: 'aircraft_status/load',
        lookups: 'aircraft_status/lookups',
      }),
      title() {
        if (this.user_name) {
  ...
  console.log('dddd');
    ...
    methods: {
      ...
      open(id) {
        window.open(this.$router.resolve({ name: 'aircraft_status-edit', params: { id } }).href, '_self');
      },
      refresh(query) {
      //   query = { ...query, type: this.type };
        this.$store.dispatch('aircraft_status/items' , {query:query});
      },
      submit() {
      ...
      remove(aircraft_statusId) {
        this.$swal({
          ...
          if (result.value) {
            this.$store.dispatch('aircraft_status/remove', aircraft_statusId).then(_ => {
              this.refresh();
              this.$swal({
                icon: 'success',
                text: "Aircraft status is deleted successfully",
                showConfirmButton: false,
                timer: 1500,
              });
            });
