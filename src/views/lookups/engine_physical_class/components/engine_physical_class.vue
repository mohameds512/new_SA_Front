...
<!--            </b-link>-->
    ...
    <template #action>
      <b-button  :to="{ name: 'EnginePhysicalClassAdd' }" v-b-tooltip.hover="$t('Global.add')" variant="primary" class="btn-icon mr-1">
...
export default {
  ...
  computed: {
    ...mapGetters({
      items: 'engine_physical_class/items',
      total: 'engine_physical_class/total',
      load: 'engine_physical_class/load',
      lookups: 'engine_physical_class/lookups',
    }),
    title() {
      if (this.user_name) {
...
console.log('dddd');
  ...
  methods: {
    ...
    open(id) {
      window.open(this.$router.resolve({ name: 'engine_physical_class-edit', params: { id } }).href, '_self');
    },
    refresh(query) {
    ...
    remove(engine_physical_classId) {
      this.$swal({
        ...
        if (result.value) {
          this.$store.dispatch('engine_physical_class/remove', engine_physical_classId).then(_ => {
            this.refresh();
            this.$swal({
              icon: 'success',
              text: "Engine physical class is deleted successfully",
              showConfirmButton: false,
              timer: 1500,
            });
          });
