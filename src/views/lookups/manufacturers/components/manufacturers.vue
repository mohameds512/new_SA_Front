<template>
  ...
  <!--            </b-link>-->
      ...
      <template #action>
        <b-button  :to="{ name: 'manufacturers-add' }" v-b-tooltip.hover="$t('Global.add')" variant="primary" class="btn-icon mr-1">
  ...
  export default {
    ...
    computed: {
      ...mapGetters({
        ...
        load: 'manufacturers/load',
        lookups: 'manufacturers/lookups',
      }),
      title() {
        if (this.user_name) {
  ...
  console.log('dddd');
    ...
    methods: {
      ...
      open(id) {
        window.open(this.$router.resolve({ name: 'manufacturers-edit', params: { id } }).href, '_self');
      },
      refresh(query) {
      ...
      remove(userId) {
        this.$swal({
          ...
          if (result.value) {
            this.$store.dispatch('manufacturers/remove', userId).then(_ => {
              this.refresh();
              this.$swal({
                icon: 'success',
                text: "Manufacturer is deleted successfully",
                showConfirmButton: false,
                timer: 1500,
              });
            });
      ...
      open_role(id) {
        window.open(this.$router.resolve({ name: 'manufacturers-role', params: { id: id } }).href, '_blank');
      },
      restore(userId) {
        this.$swal({
          title: `${this.$t('Global.deleteTitle')}`,
