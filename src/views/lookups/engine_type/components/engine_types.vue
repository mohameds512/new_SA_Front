...
<!--            </b-link>-->
    ...
    <template #action>
      <b-button  :to="{ name: 'engine_types-add' }" v-b-tooltip.hover="$t('Global.add')" variant="primary" class="btn-icon mr-1">
...
export default {
  ...
  computed: {
    ...mapGetters({
      items: 'engine_type/items',
      total: 'engine_type/total',
      load: 'engine_type/load',
      lookups: 'engine_type/lookups',
    }),
    title() {
      if (this.user_name) {
...
console.log('dddd');
  ...
  methods: {
    ...
    open(id) {
      window.open(this.$router.resolve({ name: 'engine_types-edit', params: { id } }).href, '_self');
    },
    refresh(query) {
    ...
    remove(engine_type_id) {
      this.$swal({
        ...
        if (result.value) {
          this.$store.dispatch('engine_type/remove', engine_type_id).then(_ => {
            this.refresh();
            this.$swal({
              icon: 'success',
              text: "Engine type is deleted successfully",
              showConfirmButton: false,
              timer: 1500,
            });
          });
    ...
    open_role(id) {
      window.open(this.$router.resolve({ name: 'engine_types-role', params: { id: id } }).href, '_blank');
    },
    restore(userId) {
      this.$swal({
        title: `${this.$t('Global.deleteTitle')}`,
