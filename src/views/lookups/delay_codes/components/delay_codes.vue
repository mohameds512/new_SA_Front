<template>
  ...
  <!--            </b-link>-->
      ...
      <template #action>
        <b-button :to="{ name: 'DelayCodesAdd' }" v-b-tooltip.hover="$t('Global.add')" variant="primary" class="btn-icon mr-1">
  ...
  export default {
    ...
    computed: {
      ...mapGetters({
        items: 'delay_codes/items',
        total: 'delay_codes/total',
        load: 'delay_codes/load',
        lookups: 'delay_codes/lookups',
      }),
      title() {
        if (this.user_name) {
  ...
  console.log('dddd');
    ...
    methods: {
      ...
      open(id) {
        window.open(this.$router.resolve({ name: 'delay_codes-edit', params: { id } }).href, '_self');
      },
      refresh(query) {
      //   query = { ...query, type: this.type };
        this.$store.dispatch('delay_codes/items' , {query:query});
      },
      submit() {
      ...
      remove(delay_codeId) {
        this.$swal({
          ...
          if (result.value) {
            this.$store.dispatch('delay_codes/remove', delay_codeId).then(_ => {
              this.refresh();
              this.$swal({
                icon: 'success',
                text: "Delay code is deleted successfully",
                showConfirmButton: false,
                timer: 1500,
              });
            });
