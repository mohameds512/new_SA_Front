<template>
  ...
  <!--            </b-link>-->
      ...
      <template #action>
        <b-button  :to="{ name: 'fleet_types-add' }" v-b-tooltip.hover="$t('Global.add')" variant="primary" class="btn-icon mr-1">
  ...
  export default {
    ...
    computed: {
      ...mapGetters({
        items: 'fleet_type/items',
        total: 'fleet_type/total',
        load: 'fleet_type/load',
        lookups: 'fleet_type/lookups',
      }),
      title() {
        if (this.user_name) {
      ...
      fields() {
        let fields = [
          {
            key: 'aircraft_family_id.registration',
          ...
          {
            key: 'engine_physical_class_id.name',
  ...
  console.log('dddd');
    ...
    methods: {
      ...
      open(id) {
        window.open(this.$router.resolve({ name: 'fleet_types-edit', params: { id } }).href, '_self');
      },
      refresh(query) {
      ...
      remove(typeId) {
        this.$swal({
          ...
          if (result.value) {
            this.$store.dispatch('fleet_type/remove', typeId).then(_ => {
              this.refresh();
              this.$swal({
                icon: 'success',
                text: "Fleet type is deleted successfully",
                showConfirmButton: false,
                timer: 1500,
              });
            });
      ...
      open_role(id) {
        window.open(this.$router.resolve({ name: 'fleet_types-role', params: { id: id } }).href, '_blank');
      },
      restore(userId) {
        this.$swal({
          title: `${this.$t('Global.deleteTitle')}`,
