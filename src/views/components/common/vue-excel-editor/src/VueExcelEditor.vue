<template>
  <div class="vue-excel-editor" :style="{ 'max-width': width}">
    <div class="component-content">
          ...
          <thead class="center-text">
            <tr>
              ...
              <th class="center-text first-col tl-setting"
                  >
                <span style="width:100%" :class="{hide: noNumCol}"
              ...
              <th v-for="(item, p) in fields"
                ...
                <div  v-show="!item.invisible"
                  ...
                  @contextmenu.prevent="panelFilterClick(item)" style="width:50px; text-align:center;">
                  <br/>
                  <!---<CIcon name="cil-sort-alpha-down" height="36" /> -->
            ...
            <tr v-else 
              ...
              <template v-for="(item, p) in fields">
                <td v-show="!item.invisible" 
                    ...
                    <div  v-if="item.badge"><router-link :to="`/students/${record.student_id}`"  target="_blank" class="pointer d-flex justify-content-between" style="cursor:pointer !important; text-decoration:underline;" variant="light-primary">
                      <!-- <b-badge  :value="item.badge.name"  type="warning" v-if="item.badge">
                      </b-badge> -->
                      <span class="pointer">{{  item.toText(record[item.name]) }} </span>
                      
                      <div v-if="item.badge" v-for="(badge,index) in item.badge" :key="index"> 
                      
                      <b-badge v-for="(ba,index) in record.status" :variant="getbadge(ba.name)" v-if="$i18n.locale == 'en'" style="float:right; display:flex;" >
                          {{ba.name}}&nbsp; 
                          
                          <br>
                       </b-badge>
                      <b-badge v-for="(ba,index) in record.status" :variant="getbadge(ba.name)" v-if="$i18n.locale == 'ar'" style="float:left; display:flex;" >
                          {{ba.name_local}}&nbsp;
                    ...
                    <div v-else class="float-left">{{  item.toText(record[item.name]) }} </div>
      ...
      <!-- Footer -->
      <div ref="footer" class="footer center-text" :class="{hide: noFooter}" style="position:relative; margin-left:0px;" @mousedown="ftMouseDown">
...
export default {
    ...
    localizedLabel: {
      ...
      default() {
        return {
          ...
          sortAscending:
            this.$i18n.locale === "en" ? "Sort Ascending" : "ترتيب تصاعدي",
          sortDescending:
            this.$i18n.locale === "en" ? "Sort Descending" : "ترتيب تنازلي",
          near: "≒ Near",
          exactMatch: this.$i18n.locale === "en" ? "= Exact Match" : "=  يطابق",
          notMatch: this.$i18n.locale === "en" ? "≠ Not Match" : "≠ لا يطابق ",
          greaterThan:
            this.$i18n.locale === "en" ? "&gt; Greater Than" : "&gt; اكبر من",
          greaterThanOrEqualTo:
            this.$i18n.locale === "en"
              ? "≥ Greater Than or Equal To"
              : "اكبر من او يساوي",
          lessThan:
            this.$i18n.locale === "en" ? "&lt; Less Than" : "&lt; اقل من",
          lessThanOrEqualTo:
            this.$i18n.locale === "en"
          ...
          customFilter:
            this.$i18n.locale === "en" ? "Custom Filter" : "تصنيف مخصص",
          ...
          noRecordIndicator:
            this.$i18n.locale === "en" ? "No record" : "لا يوجد بيانات",
        };
      },
    },
    recordFilter: {
  ...
  methods: {
    ...
    getbadge(badge) {
      if (badge == "Registered") return "light-info";
      else if (badge == "Fresh") return "light-info";
      else if (badge == "Succeeded") return "light-success";
      else if (badge == "Repeat") return "light-secondary";
      else if (badge == "Improve") return "light-primary";
      else if (badge == "Elective") return "light-primary";
      else if (badge == "Drop Request") return "light-warning";
      else if (badge == "Register Request") return "light-warning";
      else if (badge == "Withdraw Request") return "light-warning";
      else if (badge == "Dropped") return "light-danger";
      else if (badge == "Failed") return "light-danger";
      else if (badge == "Withdrawn") return "light-danger";
    },
    ...
    sort(n, pos) {
      ...
      const field = this.fields[colPos == -1 ? 0 : colPos];
    ...
    exportTable(format, selectedOnly, filename) {
      ...
      setTimeout(() => {
        ...
        const labels = this.fields.map((field) => field.name);
...
.table-content {
  flex: 1 1 auto;
  font-size: 2rem;
  text-shadow: 0.3px 0.3px 1px rgb(179, 168, 168);
  overflow: scroll;
...
.table-content::-webkit-scrollbar-thumb {
  background: rgba(94, 91, 91, 0.933);
}
.table-content::-webkit-scrollbar-thumb:hover {
  background: rgba(148, 141, 141, 0.6);
}
.table-col-header {
  cursor: text;
  color: rgb(10, 10, 10);
...
.systable thead th {
  ...
  cursor: s-resize;
  z-index: 6;
  border-bottom: 1px solid rgb(209, 200, 200);
}
/*
